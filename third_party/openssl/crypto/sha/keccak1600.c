...
void SHA3_squeeze(
     ...
     */
    #if !defined(KECCAK_REF) && !defined(KECCAK_1X) &&                         \
        !defined(KECCAK_1X_ALT) && !defined(KECCAK_2X) &&                      \
        ...
        /*
         * These don't have "and with complement" instruction, so minimize
         * amount of "not"-s. Implemented only in the [default] KECCAK_2X
         * variant.
         */
...
static unsigned char const rhotates[5][5] = {
    {0, 1, 62, 28, 27},
    {36, 44, 6, 55, 20},
    {3, 10, 43, 25, 39},
    {41, 45, 15, 21, 8},
...
void SHA3_sponge(
    unsigned char const *inp, size_t len, unsigned char *out, size_t d,
    size_t r)
{
    uint64_t A[5][5];
...
int main(void)
{
    ...
    size_t i;
    static unsigned char const result[512] = {
